...
def lazy_process_import_file(
    file_path: str = "src/tmp/data/sonotheque1.csv", num_rows: int = 10000
):
    logger = get_task_logger(__name__)
    media_conversion_mapper = ImportConversionMapper(
        schema_version=1
    ).get_media_mapping()
    ...
    lf = lf.rename(media_conversion_mapper).select(
        [pl.col(new_name) for new_name in media_conversion_mapper.values()]
    )
    # Convert "last_modified_date" and "creation_date" columns to Polars temporal Datetime format
    lf = lf.with_columns(
        [
            pl.col("last_modified_date")
            .str.strptime(pl.Datetime, "%Y-%m-%d %H:%M:%S%.3f")
            .alias("updated_at"),
    ...
    aggregated_lf = lf.group_by("son_primary_key").agg(
        [
            ...
            if column != "son_primary_key"
        ]
    )

    medias = (
        pt.DataFrame(aggregated_lf.collect())
        .set_model(Media)
        .drop()
